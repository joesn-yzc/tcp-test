...
char str_to_send[200] =
    "important notice:to cerebrate China log on moon successful on 12nd,Jan,2010. everyone has a day free from work!\n";
...
//bind , the ip and port information is aready in the sockaddr
	...
	while (1)
	{
		...
		if (-1 ==
		(sockfd_client =
		 accept(sockfd, (struct sockaddr *) &client_sockaddr,
			&sin_size))) {
		perror("accept");
		exit(1);
		}
		...
		if ((new_pid = fork())==0)
		{
			...
			inet_ntop(AF_INET,(void *)&client_sockaddr.sin_addr,client_IP,sizeof(client_IP));
